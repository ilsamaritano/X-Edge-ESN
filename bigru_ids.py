...
@torch.no_grad()
def measure_bigru_inference(model, loader, device, n_warmup: int = 5):
    model.eval()
...
def run_bigru_toolkit(
    ...
    log_path = output_dir / "bigru_train.log"
    ...
    for run in range(1, num_reinit + 1):
        ...
        timing = measure_bigru_inference(model, test_loader, device)
        ...
        ckpt_path = output_dir / f"bigru_run{run}.pt"
        torch.save({
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run BiGRU IDS experiment over multiple random initializations.")
